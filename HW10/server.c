...
char * clientComm(int clntSockfd,int * senderBuffSize_addr, int * optlen_addr){
    ...
    /*
    //the problem was that there was no data validation step checking
    //to see if the size of senderBuffSize_addr exceeds MAX_DATA_SIZE
    //by implementing a simple if statement, we have created a
    //sufficient way to prevent buffer overflow
    if(*senderBuffSize_addr > MAX_DATA_SIZE){
        fprintf(stderr, "ERROR, max byte count exceeded!\n");
        exit(1);
    }
    */
    strcpy(str, recvBuff);
