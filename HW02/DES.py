...
#######################################################################
# Citation: Lecture 3 code, Hw2_starter.py, generate_round_keys.py: Avinash Kak
#######################################################################
...
class DES ():
    ...
    # class would need.
        ...
        self.p_box_permutation = [15,6,19,20,28,11,27,16,0,14,22,25,4,17,30,9,1,7,23,13,31,26,2,8,18,12,29,5,21,10,3,24]
        f = open(key, 'r')
        self.key = (BitVector(textstring = f.read())).permute(self.key_permutation_1)
    ...
    #assuming all files have already been opened in main
        round_key = self.generate_round_keys( self.key )
        bv = BitVector( filename=message_file )
        encrypted_file = open(outfile,'w')
        final_block = BitVector(size=0)
        while (bv.more_to_read):
            bitvec = bv.read_bits_from_file( 64 )
            if bitvec.length() > 0:
                bitvec.pad_from_right(64 - len(bitvec))
                [LE, RE] = bitvec.divide_into_two()
                for iter in round_key:
                    newRE = RE.permute( self.expansion_permutation )
                    out_xor = newRE ^ iter
                    out_sub = self.substitute(out_xor)
                    RE_modified = out_sub.permute(self.p_box_permutation)
                    temp = LE
                    LE = RE
                    RE = RE_modified ^ temp
                final_block += RE + LE
        final_hex = final_block.get_hex_string_from_bitvector()
        encrypted_file.write(final_hex)
    ...
    # writes the recovered plaintext to the outfile
        round_key = self.generate_round_keys( self.key )
        round_key.reverse()
        encrypted = open(encrypted_file,'r')
        bv = BitVector(hexstring=encrypted.read())
        bvout = BitVector(filename=outfile)
        final_block = BitVector(size=0)
        for i in range(0, bv.size // 64):
            ...
            if bitvec.length() > 0:
                ...
                for iter in round_key:
                    ...
                    LE = temp
                final_block += RE + LE
        final_hex = final_block.get_text_from_bitvector()
        bvout.write_to_file(final_hex)
        encrypted.close()

        return
    
    def encrypt_ppm(self, ppm_file, outfile):
    # encrypts the contents of the message file and writes
    # the ciphertext to the outfile
    #assuming all files have already been opened in main
        round_key = self.generate_round_keys( self.key )
        bv = BitVector( filename=ppm_file )
        encrypted_file = open(outfile,'wb')

        header = bv.read_bits_from_file(17 * 8)
        header.write_to_file(encrypted_file)
        ...
        while (bv.more_to_read):
            ...
            if bitvec.length() > 0:
                ...
                for iter in round_key:
                    ...
                    out_xor = newRE ^ iter
                    out_sub = self.substitute(out_xor)
                    feisteled_RE = out_sub.permute(self.p_box_permutation)
                    old_RE = RE
                    RE = feisteled_RE ^ LE
                    LE = old_RE
                outfile = RE + LE
                outfile.write_to_file(encrypted_file)
        encrypted_file.close()
        return
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "-e":
        ...
        cipher.encrypt_ppm(sys.argv[2], sys.argv[4])
